//camera moves when player goes towards border
...
function Update(){
	...
	if(moveScreenRight){
		camera.transform.position.x = Mathf.SmoothDamp(camera.transform.position.x, camera.transform.position.x + borderX, velocity.y, smoothTime ); //adj 
	}
	if(cameraTarget.transform.position.x < camera.transform.position.x - borderX && moveDir.moveDirection == 1){
