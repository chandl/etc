...
var canBoost						: boolean 			= true;						// enabled sprints
...
var speedBoost						: float				= 20.0;						// maximum sprint speed
...
function UpdateMoveDirection 	() {												// motor, ani, and direction of player			
	...
	if ( IsGrounded () )															// if player on ground
	{
		...
		Boost 			();															// check for player sprinting
	...
	{										
		...
		Fall ();
		JumpFromAir();															// check if player is falling from jump		
	}
...
function Sprint 				() {												// sprints player
	if( canSprint )
	{
		if( moveSpeed > speedRun && moveSpeed <= speedSprint && Input.GetButton("Fire1"))												// check that speed is greater than sprint speed
		{
			animation.CrossFade ( aniSprint.name ); 								//play animation
			Message ( "Ani State: Sprint " );										//print current animation state
		}
	}
} 
function Boost					() {												// boosts player faster than sprint
	if( canBoost )
	{
...
function JumpPad				() {												// jump from crouch position
	if(canJumpFromPad)
	{
		if(jumpingFromPad)
		{
			...
			animation.CrossFade (aniJumpFall.name);
			currentJumpHeight = jumpFromCrouch;
			inAirVelocity.y = currentJumpHeight;
...
function AngleSlide				() {												// sliding if slope (angle) too much
	if ( canAngleSlide )
	{
		...
		if(slideDirection.magnitude >0)
		{
			...
			animation.CrossFade (aniSlide.name);
...
function Hurt					() {												// player hurt by enemy objects
	if(canHurt)
	{
		...
		{
			Message("Need colliderHurt assigned");
			return;
		}
...
function OnTriggerEnter ( other : Collider ) {										// trigger events for coin, key, bridge, jumpPad
	...
	if(other.tag == "jumpPad" && other.animation != null)
	{
	...
	if(other.tag == "key")
	{
		key += 1;
		Destroy(other.gameObject);
		Message ("You Have Collected " + coin + " keys");
	}
	if(other.tag == "bridge")
	{
		if( key < 2)
